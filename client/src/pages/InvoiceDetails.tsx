...
function InvoiceDetails() {
  ...
  const [printSize, setPrintSize] = useState<"a4" | "a5-single" | "a5-double">("a4");
  ...
  // ✅ Print size state: "a4" | "a5-single" | "a5-double"
  // (declared here for JSX use below — add to useState block above if preferred)
    ...
    ">
      ...
      <div className="mx-auto w-full mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 bg-white border rounded-lg px-3 py-2 shadow-sm">
          <span className="text-sm font-medium text-gray-600">Print Size:</span>
          <select
            value={printSize}
          ...
          >
            ...
            <option value="a5-single">A5 — Single Copy</option>
            <option value="a5-double">A5 — 2 Copies on 1 Page</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Download PDF */}
          <PDFDownloadLink
            className="inline-block rounded-lg bg-green-600 px-4 py-2 text-white hover:bg-green-700 text-sm font-medium"
            document={
              <InvoicePDF
                invoice={{ ...invoice, tenantId: tenantDetails }}
                size={printSize}
              />
            }
            fileName={`${invoice.invoiceNumber}-${printSize}.pdf`}
          >
            {({ loading }) =>
              loading ? "Preparing..." : "⬇ Download PDF"
            }
          ...
          >
            {({ url, loading }) => (
              <button
                onClick={() => handlePrint(url)}
                disabled={loading}
                className="flex items-center gap-2 rounded-lg bg-gray-100 px-4 py-2 font-medium text-gray-700 hover:bg-gray-200 text-sm"
              >
          ...
          <button
            type="button"
            onClick={() => setOpenPaymentModal(true)}
            className="rounded-lg bg-slate-900 px-4 py-2 text-white hover:bg-slate-800 text-sm font-medium"
          >
            💳 Record Payment
          </button>
