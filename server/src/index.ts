...
app.use("/api/admin",      adminRoutes);      // ✅ NEW — subscriptionRoutes HATA DIYA
...
cron.schedule("0 0 * * *", async () => {
  ...
  console.log(`Expired ${result.modifiedCount} tenants`);
});

// ─── Cron: 15-day outstanding WhatsApp reminder ───────────────────────────────
...
app.listen(PORT, async () => {
  ...
  for (const tenant of tenants) {
    console.log(`Restoring WhatsApp for ${tenant.companyName}`);
    getOrCreateClient(tenant._id.toString());
  }
});
